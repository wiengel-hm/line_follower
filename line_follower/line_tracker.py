...
from line_follower.utils import to_surface_coordinates, read_transform_config, parse_predictions, get_base, detect_bbox_center, draw_circle, get_bounding_boxes, is_visible
from ros2_numpy import image_to_np, np_to_image, np_to_pose, np_to_image, scan_to_np, to_detection3d_array, to_label_info
...
class LineFollower(Node):
    def __init__(self, model_path, config_path):
        ...
        objects_3d = ['center_bike', 'left_bike', 'right_bike', 'sign'] # Classes to track 
        self.objects_3d = {id: lbl for id, lbl in self.id2label.items() if lbl in objects_3d}

        objects_2d = ['left', 'right']
        self.objects_2d = {id: lbl for id, lbl in self.id2label.items() if lbl in objects_2d}

        # TODO make it more professional
        self.lane_width = 0.7 # m
        self.bike_offset = 0.5 # m
        self.bike_threshold = 0.5 # m

        # Get required ids
        self.bike_left_id = self.label2id['bike_left']
        self.bike_right_id = self.label2id['bike_right']
        self.bike_center_id = self.label2id['bike_center']
        self.right_id = self.label2id['right']
        self.left_id = self.label2id['left']

        # Left lane dictionary for parse_predicition
        self.left_lane = {self.label2id['left']: self.id2label[self.label2id['left']]}

        # Right lane dictionary for parse_predicition
        self.right_lane = {self.label2id['right']: self.id2label[self.label2id['right']]}

        # Center bike dictionary for parse_prediction
        self.center_bike = {self.label2id['bike_center']: self.id2label[self.label2id['bike_center']]}
    ...
    def image_callback(self, msg):
        ...
        # Extract x, y from detections
        bike_left_det = detections.get(self.bike_left_id, [None, 0.0, float('inf'), float('inf'), 0.0])
        bike_right_det = detections.get(self.bike_right_id, [None, 0.0, float('inf'), float('inf'), 0.0])
        bike_center_det = detections.get(self.bike_center_id, [None, 0.0, float('inf'), float('inf'), 0.0])

        bike_left_dist  = np.hypot(bike_left_det[2], bike_left_det[3])
        bike_right_dist = np.hypot(bike_right_det[2], bike_right_det[3])
        bike_center_dist = np.hypot(bike_center_det[2], bike_center_det[3])
        ...
        if is_visible(predictions, self.bike_left_id) and bike_left_dist <= 1.0:
            success, mask, scores = parse_predictions(predictions, self.left_lane)
            offset = 0.6
            self.get_logger().info("Bike detected on left, offsetting from left lane")

        elif is_visible(predictions, self.bike_right_id) and bike_right_dist <= 1.0:
            success, mask, scores = parse_predictions(predictions, self.right_lane)
            offset = -0.6
            self.get_logger().info("Bike detected on right, offsetting from right lane")

        elif is_visible(predictions, self.bike_center_id) and bike_center_dist <= 1.5:
            success, mask, scores = parse_predictions(predictions, self.center_bike)
            offset = 0.0
            self.get_logger().info("Bike detected on center, stopping")

        elif is_visible(predictions, self.right_id):
            success, mask, scores = parse_predictions(predictions, self.right_lane)
            offset = -self.lane_width / 2
            self.get_logger().info("Right lane detected, offsetting from right lane")

        else:
            success, mask, scores = parse_predictions(predictions, self.left_lane)
            offset = self.lane_width/2
            self.get_logger().info("Left lane detected, offsetting from left lane")

        # Identify the next waypoint along the lane line
        # Accpets deictionaries only
